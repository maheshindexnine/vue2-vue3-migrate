<template>
  <div class="container-fluid pr-0 pl-0">
    <div class="row no-gutters">
      <div class="col-md-12 grid-image-container">
        <div class="grid-image-text">
          ...
          <h3 class="sub-header">Your favourite shoe store online</h3>
          <button v-scroll-to="'#product-grid'" class="btn btn-outline-light btn-lg mt-5">
            New collection
          </button>
        </div>
      </div>
    ...
    <div class="row no-gutters">
      <div class="col-md-4 col-sm-4" v-for="item in headerImages" :key="item.id">
        ...
        <div class="grid-image-text-bottom pb-4">
          ...
          <button class="btn btn-sm mt-2 button-accent">{{ item.button }}</button>
        </div>
      </div>
    </div>
  </div>
...
<script setup>
import headerImages from '@/services/headerImages'
</script>
